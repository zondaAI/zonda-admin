...
function graphTrace(options) {
	...
	// 图片资源地址
	var imageUrl = G_CTX_ROOT + "/workflow/process/resource/process-instance?pid=" + opts.pid + "&type=image";
	
	// 获取节点流程跟踪信息
	$.getJSON(G_CTX_ROOT + '/workflow/process/trace?pid=' + opts.pid, function(infos) {
		var positionHtml = "";
		// 生成图片
		...
		if ($('#workflowTraceDialog').length == 0) {
			$('<div/>', {
				...
				html : "<div><img src='" + imageUrl + "' style='position:absolute; left:0px; top:0px;' />" + "<div id='processImageBorder'>" + positionHtml + "</div>" + "</div>"
			}).appendTo('body');
		} else {
		...
		$('#changeImg').live('click', function() {
			...
			$('<div/>', {
				...
				html : "<img src='" + G_CTX_ROOT + '/workflow/process/trace/auto/' + opts.pid + "' />"
		...
		$('#diagram-viewer').live('click', function() {
			...
			var url = G_CTX_ROOT + '/static/activiti/diagram-viewer/index.html?processDefinitionId=' + opts.pdid + '&processInstanceId=' + opts.pid;
			$('<div/>', {
				'id' : 'imgDialog',
