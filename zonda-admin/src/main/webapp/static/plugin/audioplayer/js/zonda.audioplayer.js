/**
 * Created by zonda on 2016/11/10.
 */
;
(function(window, document) {
	...
	skPlayer = function(options) {
		...
		if (Array.isArray(options.music)) {
			...
			// 初始化
			var music = options.music, target = document.getElementById('skPlayer'), HTMLcontent = '<audio src="' + music[0].src + '" preload="auto"></audio>';
			...
			HTMLcontent += '    </div>';
			HTMLcontent += '</div>';
			HTMLcontent += '<div class="skPlayer-list">';
			HTMLcontent += '	<ul class="skPlayer-list-title">';
			HTMLcontent += '    	<li data-index="-1" class="skPlayer-list-title-li">';
			HTMLcontent += '        	<i class="skPlayer-list-sign"></i>';
			HTMLcontent += '        	<span class="skPlayer-list-index">序号</span>';
			HTMLcontent += '        	<span class="skPlayer-list-name">歌曲标题</span>';
			HTMLcontent += '        	<span class="skPlayer-list-author"">歌手名称</span>';
			HTMLcontent += '    	</li>';
			HTMLcontent += '	</ul>';
			HTMLcontent += '	<ul class="skPlayer-list-music">';
			for ( var item in options.music) {
			...
			var audio = target.querySelector('audio'), playBtn = target.querySelector('.skPlayer-play-btn'), currentTime = target.querySelector('.skPlayer-percent').querySelector('.skPlayer-line'), totalTime = target.querySelector('.skPlayer-percent'), currentVolume = target.querySelector('.skPlayer-volume').querySelector('.skPlayer-line'), totalVolume = target.querySelector('.skPlayer-volume').querySelector('.skPlayer-percent'), quietVolume = target.querySelector('.skPlayer-icon'), currentTime_text = target.querySelector('.skPlayer-cur'), totalTime_text = target.querySelector('.skPlayer-total'), cover = target.querySelector('.skPlayer-cover');
			if (Array.isArray(music)) {
				var musicItem = target.querySelectorAll('.skPlayer-list-music li');
				var listSwitch = target.querySelector('.skPlayer-list-switch');
				target.classList.add('skPlayer-list-on');
			}
			
			target.parentNode.setAttribute('style', 'height: ' + ($(document).height()-10) + 'px; background-image: url(' + music[0].cover + ');');
			...
			// 初始化
			var music = options.music, target = document.getElementById('skPlayer'), HTMLcontent = '<audio src="' + music.src + '" preload="auto"></audio>';
			HTMLcontent += '<div class="skPlayer-picture">';
			...
			target.parentNode.setAttribute('style', 'height: ' + ($(document).height()-10) + 'px; background-image: url(' + music.cover + ');');
			...
			Public.ajax({
				...
				success : function(responseText) {
					...
					musicItem = target.querySelectorAll('.skPlayer-list-music li');
		...
		function audioEnd() {
			if (Array.isArray(music) && music.length !== 1) {
				...
				if (index < music.length) {
					...
					target.querySelector('.skPlayer-list-music li').classList.add('skPlayer-curMusic');
		...
		function changeMusic() {
			if (!this.classList.contains('skPlayer-curMusic')) {
				...
				target.querySelector('.skPlayer-cover').src = data.cover;
				audio.src = data.src;
				target.parentNode.setAttribute('style', 'height: ' + ($(document).height()-10) + 'px; background-image: url(' + data.cover + ');');
				if (playBtn.classList.contains('skPlayer-pause')) {
					audio.play();
				}
				currentTime_text.innerHTML = '00:00';
		...
		function handleEvent() {
			...
			if (Array.isArray(music)) {
				...
				target.querySelector('.skPlayer-list-music li:nth-child(1)').classList.add('skPlayer-curMusic');
