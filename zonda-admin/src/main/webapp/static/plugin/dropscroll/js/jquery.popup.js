/*
 * jQuery版弹窗 - jquery.popup.js-v1.0 beta (UTF-8)
 * @Author	zhujl@c-platform.com
 * @Create	Thu November 09 2016 15:30:45 GMT+0800 (中国标准时间)
 */
;
(function(w, $){
	var utils = {
	...
	Factory.prototype = {
		options: {
			...
			skinPath: "/static/plugin/dropscroll/images/"	// css或image路径
		},
		init: function(){
			...
			c.html(
				...
				/** 窗体 */
				+ '<div id="xy-ui-wrapper" class="xy-ui-wrapper anim-ui-wrapper">'
					/* 标题区域 */
					+ (opt.title ? '<div class="xy-ui-head"><div class="xy-ui-title">'+opt.title+'</div><a class="xy-ui-close"></a></div>' : '')
					/* 内容区域 */
					+ '<div class="xy-ui-body">'
					+	(opt.type ? '<div style="text-align:center;"><span style="background:url('+ opt.skinPath + opt.type +'_icon.gif) no-repeat 0 center;display:inline-block;padding:0.75rem 0 0.75rem 2.625rem;min-height: 40px;min-width: 150px;">' + opt.content + '</span></div>' : opt.content)
					+ '</div>'
					/* 底部区域 */
			...
			_i.popWin = popWin = $(".xy-ui-wrapper");
			popWin.css({
			...
			/** Zepto版--begin */
			/*offsetL = ($(window).width() - document.getElementById("xy-ui-wrapper").offsetWidth) / 2;
			offsetT = ($(window).height() - document.getElementById("xy-ui-wrapper").offsetHeight) / 2;
			
			var left = _i.isfixed ? offsetL : document.documentElement.scrollLeft + offsetL;
			var top = _i.isfixed ? offsetT : document.documentElement.scrollTop + offsetT
			...
			*//**弹窗重置*//*
			_i.setPos = function(){
				if(!_i.isfixed){
					popWin.css({
						"left": $(document).scrollLeft() + ($(window).width() - document.getElementById("xy-ui-wrapper").offsetWidth) / 2,
						"top": $(document).scrollTop() + ($(window).height() - document.getElementById("xy-ui-wrapper").offsetHeight) / 2
					});
				}else{
					popWin.css({
						"left": ($(window).width() - document.getElementById("xy-ui-wrapper").offsetWidth) / 2,
						"top": ($(window).height() - document.getElementById("xy-ui-wrapper").offsetHeight) / 2
					});
				}
			}*/
			/** Zepto版--end */
	...
	var interface = (function(){
		/*
		 * ===多功能消息弹窗(合并alert和confirm)(可接收三个参数[可向左容错])=== 1. 参数1个 信息内容 2. 参数2个 {a:1~2组合[信息内容~配置参数] | b:1~3组合[信息内容~执行函数]} 3. 参数3个 {a:1~2~3组合[信息内容~定时关闭~执行函数] | b:1~2~3组合[信息内容~配置参数~执行函数]}
		 */
		...
		/*
		 * ===简单信息弹窗(可接收三个参数[可向左容错])=== 1. 参数1个 信息内容 2. 参数2个 {a:1~2组合[信息内容~配置参数] | b:1~3组合[信息内容~执行函数]} 3. 参数3个 信息内容~配置参数~执行函数
		 */
		...
		/*
		 * ===加载外部css与js=== 1. $popup.load("skin/$popup.css"); //默认方式 $popup.load("skin/$popup.css, aaa.css"); //使用逗号分隔 ("a, b, c, d......") $popup.load(["skin/$popup.css", "aaa.css"]); //使用数组形式 (["a", "b", "c", "d"......]) 2. 加载js方法同上
		 */
	...
	// 加载css文件
	interface.load("/static/plugin/dropscroll/css/jquery.popup.css");
