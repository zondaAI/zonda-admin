				...
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
							...
							<!-- 检索  -->
							<form action="workflow/process/process-instance/finished/list.do" method="post" name="processForm" id="processForm">
								...
								<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
									<thead>
										<tr>
											...
											<th class="center">流程发起人</th>
											<th class="center"><i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>流程启动时间</th>
											<th class="center"><i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>流程结束时间</th>
											...
											<c:when test="${not empty processList}">
												<c:if test="${QX.cha == 1 }">
													<c:forEach items="${processList}" var="process" varStatus="vs">
														<tr>
															<td class='center' style="width: 30px;"><label><input type='checkbox' name='ids' value="${process.id }" id="${process.processDefinitionId }" alt="${process.processDefinitionId }" class="ace" /><span class="lbl"></span></label></td>
															<td class='center' style="width: 30px;">${vs.index + 1}</td>
															<td class='center'>${process.id }</td>
															<td class='center'>${process.processDefinitionId }</td>
															<td class='center'>${process.startUserId }</td>
															<td class='center'><fmt:formatDate value="${process.startTime }" pattern="yyyy-MM-dd HH:mm:ss" /></td>
															<td class='center'><fmt:formatDate value="${process.endTime }" pattern="yyyy-MM-dd HH:mm:ss" /></td>
															<td class='center'>${empty process.deleteReason ? "正常结束" : process.deleteReason}</td>
														</tr>
													</c:forEach>
												</c:if>
												<c:if test="${QX.cha == 0 }">
